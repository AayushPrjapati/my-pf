...
export default function App() {
    ...
    const pageVariants = {
        ...
        in: { opacity: 1, y: 0, transition: { type: 'tween', ease: "easeInOut", duration: 0.5 } },
        out: { opacity: 0, y: -20, transition: { type: 'tween', ease: "easeInOut", duration: 0.3 } }
    };
    
    const contactVariants = {
        initial: { y: "-100vh", opacity: 0 },
        in: { y: 0, opacity: 1, transition: { type: 'tween', ease: 'easeInOut', duration: 0.5 } },
        out: { y: "-100vh", opacity: 0, transition: { type: 'tween', ease: 'easeInOut', duration: 0.5 } }
    };
    
    const scaleCenterVariants = {
        initial: { scale: 0.9, opacity: 0 },
        in: { scale: 1, opacity: 1, transition: { type: 'tween', ease: "easeInOut", duration: 0.5 } },
        out: { scale: 0.9, opacity: 0, transition: { type: 'tween', ease: "easeInOut", duration: 0.3 } }
    };

    const toContactVariants = {
        initial: contactVariants.initial,
        in: contactVariants.in,
        out: scaleCenterVariants.out, 
    };

    const fromContactVariants = {
        initial: scaleCenterVariants.initial,
        in: scaleCenterVariants.in,
        out: contactVariants.out,
    };
    ...
    const getVariants = () => {
        if (page.section === 'contact') return toContactVariants;
        if (prevSection === 'contact') return fromContactVariants;
        if (prevSection === 'home') return scaleCenterVariants; 
        return pageVariants;
    };
